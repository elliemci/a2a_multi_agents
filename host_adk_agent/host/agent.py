...
class HostAgent:
    """The python Host agent which is reponsible for handling all remote connections."""
    ...
    async def _async_init_components(self, remote_agent_addresses: List[str]):
        ...
        # build a list of 'business cards' for each agent, containing name and description
        agent_info = [
            json.dumps({"name": card.name, "description": card.description})
            for card in self.cards.values()
        ]
        print("agent_info:", agent_info)
        # save all the above information into self agent property, so for setting up the root instructions
        # for the agents that are available
        self.agents = "\n".join(agent_info) if agent_info else "No friends found"

    # the class method operates on the class itself rather than on an instance of the class
    @classmethod
    async def create(
        cls,  # a clss method is bound to the class object as its first argument, instead of the instance object
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            name="Host_Agent",
            instruction=self.root_instruction,  # a simple string with a list of available agents
    ...
    async def send_message(self, agent_name: str, task: str, tool_context: ToolContext):
        ...
        # Create NEW IDs for each message - don't reuse old task IDs
        message_id = str(uuid.uuid4())
        context_id = str(uuid.uuid4())

        # create the message WITHOUT specifying taskId - let the remote agent create it
        payload = {
            "message": {
                "role": "user",
        ...
        # Extract the actual task from the response
        returned_task: Task = send_response.root.result
        print(f"Task created with ID: {returned_task.id}")

        # raw text dump of the response, to create an artifacts list
                ...
                if artifact.get("parts"):
                    for part in artifact["parts"]:
                        ...
                        if isinstance(part, dict) and part.get("text"):
                            ...
                            # Keep original part structure
                            resp.append(part)

        print(f"Extracted response parts: {resp}")
        return resp
...
def _get_initialized_host_agent_sync():
    ...
    async def _async_main():
        ...
        friend_agent_urls = [
            "http://localhost:10002",  # Emms's Agent
            "http://localhost:10003",  # Mike's Agent
            "http://localhost:10004",  # Elliot's Agent
        ]

        print("initializing host agent")
        # This is the Pthon Host agent, not the ADK host agent!!!
        hosting_agent_instance = await HostAgent.create(
            remote_agent_addresses=friend_agent_urls
        )
        print("HostAgent initialized")
        # once we have created the python host agent, we can create the ADK agent inside it
...
# the adk main agent which handles all the requests, it lieves in the folder which contains all the tools
