...
def generate_emma_calendar() -> dict[str, list[str]]:
    """Generates a random calendar for Emma for the next 7 days."""
    calendar = {}
    today = date.today()
    ...
    print("Emma's calendar:", calendar)

    return calendar


EMMA_CALENDAR = generate_emma_calendar()


def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks Emma's availability for a given date range.
    ...
    Returns:
        A string listing Emma's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = EMMA_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, Emma is available at: {', '.join(available_slots)}."
                results.append(availability)
            else:
                results.append(f"Emma is not available on {date_str}.")
...
def create_agent() -> LlmAgent:
    ...
    return LlmAgent(
        ...
        name="Emma_Agent",
        instruction="""
            **Role:** You are Emma's personal scheduling assistant. 
            Your sole responsibility is to manage her calendar and respond to inquiries 
            about her availability for pickleball.
            **Core Directives:**

            *   **Check Availability:** Use the `get_emma_availability` tool to determine 
                    if Emma is free on a requested date or over a range of dates. 
