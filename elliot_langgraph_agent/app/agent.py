...
def generate_elliot_calendar() -> dict[str, list[str]]:
...
KAITLYNS_CALENDAR = generate_elliot_calendar()
...
class AvailabilityToolInput(BaseModel):
    ...
    date_range: str = Field(
        ...
        description=(
            ...
            "'2025-11-27' or '2025-11-28 to 2025-11-30'."
        ),
    )
...
def get_availability(date_range: str) -> str:
    """Use this to get Elliot's availability for a given date or date range."""
    ...
    try:
        ...
        for i in range(delta.days + 1):
            ...
            if available_slots:
                availability = (
                    f"On {date_str}, Elliot is available at: "
                    f"{', '.join(available_slots)}."
                )
                results.append(availability)
            else:
                results.append(f"Elliot is not available on {date_str}.")
...
class ElliotAgent:
    """ElliotAgent - a specialized assistant for scheduling."""

    SUPPORTED_CONTENT_TYPES = ["text", "text/plain"]

    SYSTEM_INSTRUCTION = (
        "You are Elliot's scheduling assistant. "
    ...
    def invoke(self, query, context_id):
        ...
        # invoce tha graph with the root message created above
        ...
        for item in self.graph.stream(inputs, config, stream_mode="values"):
            ...
            if (
                ...
                yield {
                    ...
                    "content": "Checking Elliot's availability...",
                }
        ...
        if structured_response and isinstance(structured_response, ResponseFormat):
            ...
            if structured_response.status == "error":
                return {
                    "is_task_complete": False,
                    "require_user_input": True,
                    "content": structured_response.message,  # the final message to be returned as a response to the client agent
                }
